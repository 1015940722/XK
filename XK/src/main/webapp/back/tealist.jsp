	...
	<body style="background:#EEE;">
		...
		<div id="content" style="margin-left:0px; min-height:400px;">
			...
			<div id="breadcrumb" style="height:40px;line-height:33px;">
				...
				<a href="#">用户管理</a>
				<a href="#" class="current"><c:if test="${lv==2}">教师管理</c:if><c:if test="${lv==3}">精品教师管理</c:if></a>
				<div style="float:right;">
					<form style="float:left;" action="findstu.html">
					...
					<div class="span12">
						...
						<div class="widget-box">
							<div class="widget-title">
								...
								<h5><c:if test="${lv==2}">教师管理</c:if><c:if test="${lv==3}">精品教师管理</c:if></h5>
							</div>
							<div class="widget-content nopadding">
								<table class="table table-bordered table-striped">
									...
									<tr>
										<td><center>${stu.uId}</center></td>
										<td><center>${stu.uName}</center></td>
										<td><center>${stu.uUsername}</center></td>
										<td><center>${stu.uPassword}</center></td>
										<td><center>
											<fmt:formatDate value="${stu.uDate}" pattern="yyyy-MM-dd hh:ss:SSS" />
											</center></td>
											<td><center>个人资料</center></td>
										<td><center><a href="deleteu.html?id=${stu.uId}" onclick= "return confirm('真的要删除吗？？？再考虑考虑？？？');"><i class="icon-trash"></i>删除</a></center></td>
									</tr>
									</c:forEach>
									</tbody>
								</table>							
							...
							<center>
								<div>
								 <form action="findstu.html" method="get" onsubmit="return gotoo()">
									<c:if test="${page==0}">
							        <h1>没有这样的存货！亲！</h1>	
							         </c:if>
							         <c:if test="${page!=0}">
