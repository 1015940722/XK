	...
	<body style="background:#EEE;">
		...
		<div id="content" style="margin-left:0px; min-height:400px;">
			...
			<div id="breadcrumb" style="height:40px;line-height:33px;">
				...
				<a href="#">视频管理</a>
				<a href="#" class="current"><c:if test="${lv==1}">视频管理</c:if><c:if test="${lv==2}">精品视频管理</c:if></a>
				<div style="float:right;">
					<form style="float:left;" action="findvedio.html">
					...
					<div class="span12">
						...
						<div class="widget-box">
							<div class="widget-title">
								...
								<h5><c:if test="${lv==1}">视频管理</c:if><c:if test="${lv==2}">精品视频管理</c:if></h5>
							</div>
							<div class="widget-content nopadding">
								<table class="table table-bordered table-striped">
									...
									<tr>
										<td><center>${vedio.vId}</center></td>
										<td><center>${vedio.vName}</center></td>
										<td><center>${vedio.vType}</center></td>
										<td><center>
											<fmt:formatDate value="${vedio.vDate}" pattern="yyyy-MM-dd hh:ss:SSS" />
											</center></td>
										<td><center>播放</center></td>
										<td><center><a href="deletev.html?id=${vedio.vId}" onclick= "return confirm('真的要删除吗？？？再考虑考虑？？？');"><i class="icon-trash"></i>删除</a></center></td>
									</tr>
									</c:forEach>
									</tbody>
								</table>							
							...
							<center>
								<div>
								 <form action="findvedio.html" method="get" onsubmit="return gotoo()">
									<c:if test="${page==0}">
							        <h1>没有这样的存货！亲！</h1>	
							         </c:if>
							         <c:if test="${page!=0}">
								        <c:if test="${page>1}">
								           <a href="findvedio.html?startpage=${page-1}">上一页</a>
								          </c:if>
								          	<span>第${page}/${pagecount}页</span>
								          <c:if test="${page<pagecount}">
								           <span><a href="findvedio.html?startpage=${page+1}">下一页</a></span>
								         </c:if>
